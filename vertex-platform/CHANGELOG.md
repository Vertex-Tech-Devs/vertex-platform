...
- **Disponibilidad autoritativa de subdominios**: el chequeo hace un probe real (crear-y-borrar sitio en Hosting, cache 5 min) contra el shard de la tienda —o el proyecto master durante la creación—; como los `SITE_ID` son únicos globalmente y no hay API de consulta, antes se mostraba un falso “Disponible”. La UI ahora muestra el motivo exacto (“Reservado en Firebase”) y sugerencias.
- **Cambio de subdominio con clon real de versión** (`versions:clone`) para evitar “Site name mismatch”, con health-check 200 y revert si falla.
