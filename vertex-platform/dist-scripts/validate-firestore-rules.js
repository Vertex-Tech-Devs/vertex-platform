...
function main() {
  ...
  console.log(
    `${colors.bright}${colors.cyan}=== Firestore Rules Sync Validator ===${colors.reset}`,
  );
  ...
  if (!storefrontTenantBlock) {
    console.error(
      `${colors.red}Error: 'match /tenants/{tenantId}' block not found in Storefront rules.${colors.reset}`,
    );
    process.exit(1);
  }
  if (!platformTenantBlock) {
    console.error(
      `${colors.red}Error: 'match /tenants/{tenantId}' block not found in Platform rules.${colors.reset}`,
    );
    process.exit(1);
  }
  ...
  if (missingInPlatform.length > 0) {
    ...
    console.error(
      `${colors.yellow}The following tenant subcollections are defined in Storefront rules but missing in Platform rules:${colors.reset}`,
    );
