...
## Trazabilidad y prueba de versiones (v0.4.0)

Para verificar que las tiendas pueden actualizarse a una versión específica de la plantilla:

1. **Bump**: `version: "0.4.0"` en `storefront/package.json` y `storefront/functions/package.json`;
   `CURRENT_TEMPLATE_VERSION = '0.4.0'` en platform. Cambios visibles opcionales (ej. badge
   de versión en el footer del shop — `HECHO CON VERTEX · v0.4.0`).
2. **Tag + release**: `git tag v0.4.0 && git push origin v0.4.0` → el workflow `Release`
   crea la GitHub Release `v0.4.0` con changelog automático.
3. **Selector**: `listTemplateVersions` muestra v0.4.0 (latest) + v0.3.0 + v0.2.0 + v0.1.0.
4. **Aplicar por tienda**: "Versión de Plantilla" → seleccionar v0.4.0 → "Aplicar versión"
   → `updateStoreVersion` → deploy desde `refs/tags/v0.4.0` → `completeVersionUpdate`
   actualiza `appVersion: "v0.4.0"` en Firestore.

### Dónde comprobar la versión activa en cada capa

| Capa | Dónde ver | Ejemplo |
|---|---|---|
| **Visual (v0.4.0+)**: footer del shop | `HECHO CON VERTEX · v0.4.0` (visible sin herramientas) |
| **Consola**: `[Vertex Storefront] v0.4.0` | DevTools → Console al cargar |
| **Header HTTP**: `x-vertex-template-version` | DevTools → Network → Doc request headers |
| **Firestore/panel**: `appVersion: "v0.4.0"`, `targetChannel: "stable"`, `lastDeployedAt` | Panel → Versión app / Canal / Último deploy |

### Prueba de aislamiento entre tiendas

1. Tienda A: aplicar **v0.4.0** → footer muestra `· v0.4.0`, panel `Versión app: v0.4.0`.
